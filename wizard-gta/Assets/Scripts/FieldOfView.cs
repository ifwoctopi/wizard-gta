...
public class FieldOfView : MonoBehaviour
{
    ...
    [Header("Owner")]
    public EnemyScript enemy; // assign the parent EnemyScript
    ...
    public bool showState = false; // toggle this in the inspector

    [Header("Detection")]
    public float partialDetectionTime = 2f;
    ...
    private void CheckPlayerDetection()
    {
        ...
        Vector3 coneForward = GetVectorFromAngle(startingAngle - fov / 2f); // world space
        ...
        if (angleToPlayer <= fov / 2f)
        {
            ...
            if (hit.collider == null || hit.collider.transform == player) // allow detection if player is hit
            {
                ...
                if (detectionTimer >= partialDetectionTime)
                {
                    if (showState)
                    {
                        visionMaterial.color = Color.red;
                        Debug.Log("Chasing");
                    }
                    enemy.StartChase(player);
                    isAlert = true;
                }
                else
                {
                    if (showState) {
                        visionMaterial.color = Color.yellow;
                        Debug.Log("Confused");
                    }
