/*
 * ==============================================================================
 * ** Victor Engine MV - MateriaSystem
 * ------------------------------------------------------------------------------
 * Version History:
...
Imported['VE - Materia System'] = '1.04';
...
/*:
 * ------------------------------------------------------------------------------
 * @plugindesc v1.04 - Attach materias to actors for parameter bonus and skills.
 * @author Victor Sant
 *
 ...
 * @help 
 * ------------------------------------------------------------------------------
 * Actors, Classes and States Notetags:
 * ------------------------------------------------------------------------------
 *
 *  <materia slots>
 ...
 *
 * ------------------------------------------------------------------------------
 * Weapons, Armors (Non-Materia) Notetags:
 * ------------------------------------------------------------------------------
 *
 ...
 *   
 * ------------------------------------------------------------------------------
 * Enemies notes Notetags:
 * ------------------------------------------------------------------------------
 *
 *  <materia ap: x>
 *  <materia ap: 'code'>
 *    Amount of AP (materia experience) given by the enemy.
 *      x : AP value. (numeric or a code)
 *
 * ------------------------------------------------------------------------------ 
 *  Plugin Commands
 * ------------------------------------------------------------------------------
 ...
 *
 *  Unequipmateria actor equip slot materia
 ...
 *  I can't ensure that for other authors plugins, althogh there is a chance
 ...
 *
 * ------------------------------------------------------------------------------
 * Example Notetags (Actor and Class):
 *
 *  <materia slots>
 *   Head: 0:0:0:0
 *   Left Hand: 0=0:0=0
 ...
 *   Equip 5: 0:0
 *  </materia slots>
 *   The actor have three personal slot set and two equipment slot sets .
 *     Head : 5 single slots.
 *     Left Hand: 4 slots, with two linked pairs.
 *     Right Hand : 3 slots, with a single linked trio.
 *     Equip 4 : slot for armors (Equipment Type 4), have 2 single slot. 
 *     Equip 5 : slot for accessories (Equipment Type 5), have 2 single slot. 
 *       The 'Equip' slots are replaced by the equipment slot if one is equiped.
 *
 *  <materia slots>
 *   Equip 1:
 *   Equip 2:
 *  </materia slots>
 *   The actor have two equipment slot set.
 *     Equip 1 : slot for weapons (Equipment Type 1). 
 *     Equip 2 : slot for shields (Equipment Type 2). 
 *
 *  <materia slots>
 ...
 *   Soul: 0:0:0, type armor, growth 0.5, icon 213, 214, 215
 *  </materia slots>
 *   The actor have three personal slot set. 
 *     Body : 1 single slot, growth x2 and type weapon.
 *     Mind : 2 slots, one linked pair, growth x1, type armor and 4 custom icons.
 *     Soul : 3 single slots, growth x0.5, type armor and 4 custom icons.
 *
 * ------------------------------------------------------------------------------
 ...
 *
 * ------------------------------------------------------------------------------
 * Example Notetags (materia armor):
 *
 *  <materia>
 *   ap: 2000, 18000, 35000
 *   price: 42000
...
(function() {
	...
	if (Imported['VE - Basic Module']) {
		...
		VictorEngine.Parameters.MateriaNumber   = Number(parameters["Max Materia Number"]) || 0;
		VictorEngine.Parameters.MaxSlotNumber   = Number(parameters["Max Slot Number"]);
		VictorEngine.Parameters.SingleSlotIcon  = Number(parameters["Single Slot Icon"]) || 0;
		VictorEngine.Parameters.LinkSlotLeft    = Number(parameters["Link Slot Left"]) || 0;
		VictorEngine.Parameters.LinkSlotCenter  = Number(parameters["Link Slot Center"]) || 0;
		VictorEngine.Parameters.LinkSlotRight   = Number(parameters["Link Slot Right"]) || 0;
		VictorEngine.Parameters.MateriaLevel    = Number(parameters["Materia Level Icon"]) || 0;
		VictorEngine.Parameters.MateriaDisplay  = Number(parameters["Materia Display Type"]) || 1;
		VictorEngine.Parameters.MateriaMenu     = String(parameters["Materia Menu Name"]).trim();
		VictorEngine.Parameters.MateriaApName   = String(parameters["Materia Ap Name"]).trim();
		VictorEngine.Parameters.ApReceived      = String(parameters["Ap Received Text"]).trim();
		VictorEngine.Parameters.MasterText      = String(parameters["Materia Master Text"]).trim();
		VictorEngine.Parameters.NextLevelText   = String(parameters["Next Level Text"]).trim();
		VictorEngine.Parameters.AbilityListText = String(parameters["Ability List Text"]).trim();
		VictorEngine.Parameters.ParamListText   = String(parameters["Param List Text"]).trim();
		VictorEngine.Parameters.EquipedText     = String(parameters["Equiped Materia Text"]).trim();
		VictorEngine.Parameters.MateriaBreeding = eval(parameters["Materia Breeding"]);
		VictorEngine.Parameters.CursorPriority  = eval(parameters["Cursor High Priority"]);
	}
	...
	VictorEngine.loadNotetagsValues = function(data, index) {
		...
		if (this.objectSelection(index, list)) VictorEngine.MateriaSystem.loadNotesEquip(data);
		var list = ['actor', 'class', 'state'];
		if (this.objectSelection(index, list)) VictorEngine.MateriaSystem.loadNotesActor(data);
		if (this.objectSelection(index, ['skill'])) VictorEngine.MateriaSystem.loadNotesSkill(data);
		if (this.objectSelection(index, ['enemy'])) VictorEngine.MateriaSystem.loadNotesEnemy(data);
		if (this.objectSelection(index, ['armor'])) VictorEngine.MateriaSystem.loadNotesMateria(data);
	};
	
	VictorEngine.MateriaSystem.loadNotesEquip = function(data) {
		data.materiaSlots = data.materiaSlots || {};
		this.processNotesEquip(data);
	};
	
	VictorEngine.MateriaSystem.loadNotesActor = function(data) {
		data.materiaSlots = data.materiaSlots || [];
		this.processNotesActor(data);
	};

	VictorEngine.MateriaSystem.loadNotesSkill = function(data) {
		data.materiaNegate = data.materiaNegate || {};
		this.processNotesSkill(data);
	};
	
	VictorEngine.MateriaSystem.loadNotesEnemy = function(data) {
		data.materiaAp = data.materiaAp || {};
		this.processNotesEnemy(data);
	};
	
	VictorEngine.MateriaSystem.loadNotesMateria = function(data) {
		data.materiaValue = data.materiaValue || {};
		this.processNotesMateria(data);
	};
	
	VictorEngine.MateriaSystem.processNotesEquip = function(data, type) {
		var match;
		var regex = new RegExp('<materia slots:[ ]*([\\d:=]+)([ ]*,[ ]*[\\w\\,\\. ]+)?[ ]*>', 'gi');
		while ((match = regex.exec(data.note)) !== null) {
			data.materiaSlots = this.processValuesEquip(data, match);
		};
	};
	
	VictorEngine.MateriaSystem.processNotesActor = function(data, type) {
		var match;
		var regex = VictorEngine.getNotesValues('materia slots');
		while ((match = regex.exec(data.note)) !== null) {
			this.processValuesActor(match, data.materiaSlots);
		};
	};
	
	VictorEngine.MateriaSystem.processNotesSkill = function(data) {
	...
	VictorEngine.MateriaSystem.processNotesEnemy = function(data) {
		var regex = new RegExp("<materia ap[ ]*:[ ]*(\\d+|'[^\']+'|\"[^\"]+\")[ ]*>", 'gi');
		while ((match = regex.exec(data.note)) !== null) {
			data.materiaAp = this.processValuesEnemy(match);
		};
	};
	
	VictorEngine.MateriaSystem.processNotesMateria = function(data) {
		var match;
		var regex = VictorEngine.getNotesValues('materia');
		while ((match = regex.exec(data.note)) !== null) {
			data.isMateria    = true;
			data.materiaValue = this.processValuesMateria(match);
		};
	};
	
	VictorEngine.MateriaSystem.processValuesEquip = function(data, match) {
		var max = VictorEngine.Parameters.MateriaNumber;
		result = {};
		result.id     = data.id;
	...
	VictorEngine.MateriaSystem.processValuesActor = function(match, data) {
		var value;
		var regex1 = new RegExp('([^\\n\\r:]+):[ ]*([\\d:=]+)?([ ]*,[ ]*[\\w\\,\\. ]+)?', 'gi');
		var regex2 = new RegExp('equip[ ]*(\\d+)', 'gi');
		while ((value = regex1.exec(match[1])) !== null) {
			var result = value[1].match(regex2);
			var object = this.processMateriasActor(value);
			if (result) {
	...
	VictorEngine.MateriaSystem.processValuesEnemy = function(match, data) {
		result = {};
		var value;
		var regex1 = new RegExp('^([+-]?\\d+)(\\%)?', 'gi');
		var regex2 = new RegExp("^('[^\']+'|\"[^\"]+\")", 'gi');
		while ((value = regex1.exec(match[1])) !== null) { result.flat = Number(value[1]) };
		while ((value = regex2.exec(match[1])) !== null) { result.code = value[1].slice(1, -1) };
		return result;
	};
	
	VictorEngine.MateriaSystem.processValuesMateria = function(match) {
		var result = {};
		var icon   = VictorEngine.Parameters.MateriaLevel;
	...
	VictorEngine.MateriaSystem.processMateriasActor = function(match) {
		var max = VictorEngine.Parameters.MateriaNumber;
		result  = {};
	...
	VictorEngine.MateriaSystem.getMateriaEffect = function(match, type) {
		...
		while ((value = regex.exec(match[1])) !== null) {  
			...
			if (value[4]) result[effect].text = value[4].slice(1, -1);
		};
		return result
	};
	...
	BattleManager.displayMateriaAp = function() {
		...
		if (ap > 0) {
			var text = VictorEngine.Parameters.ApReceived.format(ap);
	...
	Game_Action.prototype.makeDamageValue = function(target, critical) {
		...
		if (this.isSkill() && !this.subject().isEnemy()) value *= this.makeMateriaDamage();
	...
	Game_Action.prototype.applyGlobal = function() {
		...
		if (this.isSkill() && this.isLimitedUse()) {
	...
	Game_Action.prototype.isLimitedUse = function() {
		return ($gameParty.inBattle() && this.isLimitedUseMateria(this.item()));
	};
	...
	Game_Action.prototype.setAllMateriaAction = function(item) {
		this._isForAllBattle = this.subject().scopeToggle() === 'for all';
	};
	
	Game_Action.prototype.isLimitedUseMateria = function(item) {
		return !this.subject().isEnemy() && this.subject().limitedUseMateria(item);
	};
	...
	Game_Action.prototype.materiaAbsorbDamage = function(target, value) {
		var item = this.item();
		if (value > 0) this.pairAbsorb(target, value, item.id, 'hp');
		if (value > 0) this.pairAbsorb(target, value, item.id, 'mp');
	};
	...
	Game_Actor.prototype.materiaEquipSlots = function() {
		...
		return objects.reduce(function(r, obj) {
			return r.concat(obj.materiaSlots ? obj.materiaSlots : [])
		}, []);
	};
	...
	Game_Actor.prototype.refreshMateriaSlots = function(slot, i) {
		...
		if (slots) var materias = slots.materias.map(function(materia) { return materia });
	...
	Game_Actor.prototype.gainAp = function(ap) {
		this.materiaEquipSlots().forEach(function(data, i) {
			...
			slot.materias.forEach(function(materia) {
				if (materia) materia.gainAp(ap, slot.data.growth)
			})
		}, this);
		this.makePairedMateria();
		this.updateMateriaSkills();
	...
	Game_Actor.prototype.learnMateriaSkills = function(materia) {
		if (materia.isEnemySkill()) {
			materia.learn().forEach(function(skillId) {
				if (this.validMateriaSkill(skillId)) this.gainMateriaSkill(skillId);
			}, this)
		} else {
	...
	Game_Actor.prototype.makeMateriaPairs = function(slot, i) {
		...
		var obj   = {main: main};
		obj.type  = slot.type;
		if (slots[i * 2 - 1] === '=' && left  && left.isNormal())  obj.left  = left;
		if (slots[i * 2 + 1] === '=' && right && right.isNormal()) obj.right = right;
		if (obj.left || obj.right) pair.push(obj);
		return pair;
	};
	...
	Game_Actor.prototype.getMateriaCostChange = function(skillId) {
		...
		return this.pairedMateria().reduce(function(r, pair) {
			var value = object.materiaChangeCost(skillId, pair);
			return r.concat(value ? pair.main.pairRate('mp cost') : []);
		}, []);
	};
	...
	Game_Actor.prototype.getMateriasEffects = function(slots, effect, type, object) {
		return slots.materias.reduce(function(r, materia) {
			...
			return r.concat(isOk ? materia[object]() : []);
		}, []);
	};
	...
	Game_Actor.prototype.equipMateria = function(slot, index, newMateria) {
		...
		if (materiaSlot && materiaSlot.materias.length > index) {
			...
			if (oldMateria) $gameParty.gainMateria(oldMateria, 1);
	...
	Game_Actor.prototype.unequipMateria = function(slot, index) {
		...
		if (materiaSlot && materiaSlot.materias.length > index) {
			...
			if (materia) $gameParty.gainMateria(materia, 1);
	...
	Game_Actor.prototype.useAllMateria = function(item) {
		...
		for (var i = 0; i < pairs.length; i++) {
			...
			if (pair.main.isAll(item.id) && this.isAllMateriaSkill(pair, item.id)) {
				if (pair.main.limit()) pair.main.useTimes(item.id);
				return
			}
		}
	};
	...
	Game_Actor.prototype.limitedUsesOk = function(item) {
		if (!this._materiaSkills.contains(item.id)) return true;
		...
		for (var i = 0; i < materias.length; i++) {
			var materia = materias[i]
			if (materia.isLimited(item.id)) return true;
		}
		return false
	};
	...
	Game_Actor.prototype.pairHasSkill = function(pair, skillId) {
		return ((pair.left  && pair.left.hasSkill(skillId)) || 
				(pair.right && pair.right.hasSkill(skillId)))
	};
	...
	Game_Enemy.prototype.makeTotalAp = function() {
		...
		if (apValue.flat) return apValue.flat;
		if (apValue.code) return this.apValueCode(apValue.code);
		return 0
	};
	...
	Game_Party.prototype.gainItem = function(item, amount, includeEquip) {
		if (item && item.isMateria) {
			this.gainMateria(new Game_Materia(item.id), amount, includeEquip)
		} else {
	...
	Game_Party.prototype.addMateria = function(item, amount) {
		for (var i = 0; i < amount; i++) {
			if (!this.maxMaterias()) this._materias.push(item);
		}
		this.sortMaterias();
	};
	...
	Game_Party.prototype.removeMateria = function(item, amount, includeEquip) {
		...
		if (includeEquip) this.removeActorMateria(item, amount)
		this.sortMaterias();
	};
	...
	Game_Party.prototype.maxMaterias = function() {
		this._materias.length >= VictorEngine.Parameters.MateriaNumber;
	};
	...
	Game_Interpreter.prototype.pluginCommand = function(command, args) {
		...
		if (command.toLowerCase() === 'equipmateria') {	
			...
			if (actor && materia) actor.equipMateria(equip, slot, materia);
		}
		if (command.toLowerCase() === 'unequipmateria') {	
			...
			var equip = Number(eval(args[1]) - 1) || 0;
			var slot  = Number(eval(args[2]) - 1) || 0;
			if (actor) actor.unequipMateria(equip, slot);
		}
	...
	Window_MenuCommand.prototype.addMateriaCommand = function() {
		...
		var name    = VictorEngine.Parameters.MateriaMenu;
		if (this.needsMateriaCommand()) this.addCommand(name, 'materia', enabled);
	};
	...
	Window_ShopStatus.prototype.drawPossession = function(x, y) {
		...
		if (this._item.isMateria) {
			...
			this.drawText(VictorEngine.Parameters.EquipedText, x, y + height, width - possessionWidth);
			this.resetTextColor();
	...
	Window_ItemCategory.prototype.makeCommandList = function() {
		...
		if (SceneManager.isMateriaShop()) this.addCommand(VictorEngine.Parameters.MateriaMenu, 'materia');
	};
	...
	Scene_Shop.prototype.commandSell = function() {
		...
		if (this.materiaSell()) this.hideSellShowMateria();
	};
...
(function() {
	...
	Window_MateriaStatus.prototype.drawMateriaAp = function() {
		if (!this.materia().noLevel()) {
			var materia = this.materia();
			var mt = VictorEngine.Parameters.MasterText;
			...
			this.changeTextColor(this.systemColor());
			this.drawText(VictorEngine.Parameters.MateriaApName, 0, lh, w2, 'right')
			this.drawText(VictorEngine.Parameters.NextLevelText, 0, lh + fs, w2, 'right')
			this.changeTextColor(this.normalColor());
	...
	Window_MateriaStatus.prototype.drawMateriaSkills = function() {
		...
		this.changeTextColor(this.systemColor());
		this.drawText(VictorEngine.Parameters.AbilityListText, 4, y);
		this.changeTextColor(this.normalColor());
	...
	Window_MateriaStatus.prototype.drawMateriaParams = function() {
		...
		var y = this.contents.height / 3 + 8;
		this.changeTextColor(this.systemColor());
		this.drawText(VictorEngine.Parameters.ParamListText, x, y);
		this.changeTextColor(this.normalColor());
		var ln = this.linesNumber(y);
		var wd = this.abilityWidth();
		var ht = this.abilityHeight();
...
(function() {
	...
	Window_MateriaEquip.prototype.maxCols = function() {
		return VictorEngine.Parameters.MaxSlotNumber;
	};
	
	Window_MateriaEquip.prototype.displayType = function() {
		return VictorEngine.Parameters.MateriaDisplay === 1;
	};
	...
	Window_MateriaEquip.prototype.defaultSlotIcons = function(slots, i) {
		if (slots[i * 2 - 1] === '=' && slots[i * 2 + 1] === '=') {
			return VictorEngine.Parameters.LinkSlotCenter;
		}
		if (slots[i * 2 - 1] === '=' && slots[i * 2 + 1] !== '=') {
			return VictorEngine.Parameters.LinkSlotRight;
		}
		if (slots[i * 2 - 1] !== '=' && slots[i * 2 + 1] === '=') {
			return VictorEngine.Parameters.LinkSlotLeft;
		}
		return VictorEngine.Parameters.SingleSlotIcon;
	}
	...
	Window_MateriaEquip.prototype.isCurrentItemEnabled = function() {
		if (this.materia() && this.materia().locked()) return false;
	...
	Window_MateriaEquip.prototype.updateCursor = function() {
		...
		if ((this.maxCols() * this.slots().length) === 0) this.setCursorRect(0, 0, 0, 0);
	};
	...
	Window_MateriaEquip.prototype._createAllParts = function() {
		...
		if (VictorEngine.Parameters.CursorPriority) {
...
(function() {
	...
	Game_Materia.prototype.levelUp = function() {
		this._level++;
		if (VictorEngine.Parameters.MateriaBreeding && this.level() === this.max()) {
	...
	Game_Materia.prototype.traitSkills = function() {
		...
		return this.item().traits.reduce(function(r, trait) {
			...
			return r.concat(skill ? {id: trait.dataId, level: 0, rate: 0} : []);
